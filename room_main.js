...
function applyCurrentSettings() {
    ...
    // OPTIMIZACIÓN GAMA BAJA: Escalado de resolución interna para móviles de gama baja en modo Suave
    else if (State.gameSettings.calidad === 'baja') pixelRatio = isMobileUA ? 0.6 : 0.8; 
...
function applyMaterialLogic(model, categoryKey) {
    ...
    model.traverse((node) => {
        if (node.isMesh) {
            // OPTIMIZACIÓN: Reactivamos Culling en baja para que la GPU no dibuje lo que no ve.
            node.frustumCulled = isBaja ? true : false;
            ...
            if (isFoco || isFocoDia) {
                ...
                if(node.material) {
                    ...
                    // El personaje Lunari NECESITA DoubleSide obligatoriamente para que la ropa,
                    // cabello y cuerpo se vean bien desde cualquier ángulo, incluso en modo Suave.
                    if (categoryKey === 'lunari') {
                        node.material.side = THREE.DoubleSide;
                    } else {
                        // OPTIMIZACIÓN GAMA BAJA: Para otros objetos estáticos (muebles, estructura) 
                        // usamos FrontSide en modo baja para ganar rendimiento sin fallos visuales graves.
                        node.material.side = (isBaja || nodeIsStructure) ? THREE.FrontSide : THREE.DoubleSide;
                    }
...
const idleRandomFiles = ['lunari_idle2.glb', 'lunari_idle3.glb', 'lunari_idle4.glb', 'lunari_idle5.glb', 'lunari_idle6.glb'];
