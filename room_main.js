...
// --- ESTADOS GUARDADOS DE ENTORNO (Luz y Tele) ---
let lightOn = localStorage.getItem('lightState') !== 'off';
let isTvOn = localStorage.getItem('tvState') === 'on'; 

// --- LÓGICA DE SUEÑO LUNARI ---
let currentHourForSleep = new Date().getHours();
let isSleepTime = (currentHourForSleep >= 22 || currentHourForSleep < 7); // Horario de sueño 10PM - 7AM
// Si NO es hora de dormir, O si la luz está prendida, O si la tele está prendida = Despierta.
...
function updateLunariText(isDay, weatherCode) {
    ...
    if (!shouldWakeUp) {
        dialogBox.innerHTML = "<i>* Zzz... Zzz... *</i>";
        return;
    }
...
if (tvPowerBtn) {
    // Inicializar visualmente botón TV
    ...
    tvPowerBtn.addEventListener('click', () => {
        ...
        const onEffectEnded = () => {
            ...
            if (isTvOn) {
                isTvOn = false; 
                localStorage.setItem('tvState', 'off'); // Guardado en localStorage
                ...
                isTvOn = true;
                localStorage.setItem('tvState', 'on'); // Guardado en localStorage
...
// CÁLCULO DE MODELOS DE LUNARI
if (shouldWakeUp) {
    totalModelsToLoad += 3; // Lunari Despierta (1), FocoDia, Cuadro Clima
} else {
    totalModelsToLoad += 4; // Lunari Durmiendo (2), FocoDia, Cuadro Clima
}
...
// CARGAR MODELOS SEGÚN ESTADO DE LUNARI
if (shouldWakeUp) {
        ...
        if (baseAction && randomAction) {
            ...
            const onFinished = (event) => {
                if (event.action === randomAction) { 
                    randomAction.fadeOut(0.5); baseAction.reset().fadeIn(0.5).play(); currentAction = baseAction; lunariMixer.removeEventListener('finished', onFinished); 
                }
            };
...
// RESTO DE CARGAS (FocoDia, Cuadro Clima, etc.)
