...
import {
    scene, shared, getFreshUrl, applyMaterialLogic, gameSettings,
    actualizarIluminacionFocoDia, tvVideo, tvTexture, isTvOn,
    tvEffectTextureOff, tvEffectTextureOn, tvEffectVideoOff, tvEffectVideoOn,
    inventoryData, playerCoins, saveGame
} from './room_main.js';
...
loader.load(getFreshUrl('lunari_durmiendo1.glb'), (gltf) => {
    ...
    if (gltf.animations && gltf.animations.length > 0) {
        shared.lunariMixer = new THREE.AnimationMixer(lunariModel);
        baseAction = shared.lunariMixer.clipAction(gltf.animations[0]);
        baseAction.play();
...
loader.load(getFreshUrl('Lunari_Duerme_2.glb'), (gltf) => {
    if (gltf.animations && gltf.animations.length > 0 && shared.lunariMixer) {
        randomAction = shared.lunariMixer.clipAction(gltf.animations[0]);
...
setInterval(() => {
    if (!randomAction || !baseAction || !shared.lunariMixer || currentAction === randomAction) return;
    if (baseAction && randomAction) {
        ...
        const onFinished = (event) => {
            if (event.action === randomAction) {
                ...
                currentAction = baseAction;
                shared.lunariMixer.removeEventListener('finished', onFinished);
            }
        };
        shared.lunariMixer.addEventListener('finished', onFinished);
    }
}, 60000);

// ==================== FOCO DÍA ====================
loader.load(getFreshUrl('https://cdn.jsdelivr.net/gh/Archinime/ArchiPapu@main/foco_dia.glb'), (gltf) => {
    shared.focoDiaMesh = gltf.scene;
    applyMaterialLogic(shared.focoDiaMesh, 'foco_dia');
    const luz = new THREE.PointLight(0xffffff, 1, 50);
    const box = new THREE.Box3().setFromObject(shared.focoDiaMesh);
    const center = new THREE.Vector3();
    box.getCenter(center);
    luz.position.copy(center);
    luz.position.y -= 0.2;
    luz.shadow.bias = -0.005;
    luz.shadow.normalBias = 0.1;
    scene.add(luz);
    scene.add(shared.focoDiaMesh);
    shared.focoDiaMesh.visible = false;
    luz.visible = true;
    shared.luzFocoDia = luz;
...
export function loadItemForSlot(categoryKey, itemFile, isInitialLoad = false) {
    if (!itemFile) return;
    if (shared.loadedSlotMeshes[categoryKey]) {
        scene.remove(shared.loadedSlotMeshes[categoryKey]);
        disposeThreeJSObject(shared.loadedSlotMeshes[categoryKey]);
    }
    loader.load(getFreshUrl(itemFile), (gltf) => {
        ...
        if (categoryKey === 'pantalla_tv') {
            ...
            if (!shared.isTvOn) tvVideo.pause();
        }
        if (categoryKey === 'foco') {
            shared.focoMesh = model;
        ...
        if (categoryKey === 'interruptor') shared.switchMesh = model;
        scene.add(model);
        shared.loadedSlotMeshes[categoryKey] = model;
...
export async function setupWeatherVideo() {
    ...
    try {
        ...
        shared.esDeDiaLocal = (isDay === 1);
        shared.lastWeatherCode = code;
        actualizarIluminacionFocoDia();
    ...
    loader.load(getFreshUrl('cuadro.glb'), (gltf) => {
        ...
        shared.loadedSlotMeshes['cuadro'] = cuadroModel;
        checkLoading();
    }, undefined, () => checkLoading());
}
setupWeatherVideo();
